...
Requires Bash and tmux on PATH. Use the installed skill directory, not the current
project's working directory, and invoke Bash explicitly (seeded files are private,
not necessarily executable):

```bash
SKILL_DIR="${ZELINE_HOME:-$HOME/.zeline}/skills/public/tmux"
bash "$SKILL_DIR/scripts/find-sessions.sh"
bash "$SKILL_DIR/scripts/wait-for-text.sh" -t shared:0.0 -p 'ready' -F -T 15
```
